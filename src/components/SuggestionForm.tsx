        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="description">
              ...
              <span className="text-sm text-gray-500 ml-2">{formData.description.length}/200</span>
            </Label>
            <Textarea
              id="description"
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-200">Imagens (opcional)</label>
            <input
              ref={imageInputRef}
              type="file"
              accept="image/*"
            ...
            />
            <div
              className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center bg-gray-50 dark:bg-gray-800 w-full cursor-pointer hover:border-blue-400"
            ...
            >
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300">Clique para selecionar imagens (.jpg, .png, .webp)</p>
              <p className="text-xs text-gray-400 mt-1">Máximo 5MB por imagem. Você pode selecionar múltiplas imagens.</p>
            </div>
            {imageFiles.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-2">
                {imageFiles.map((file, idx) => (
                  <img key={idx} src={URL.createObjectURL(file)} alt={`preview-${idx}`} className="w-20 h-20 object-cover rounded border" />
                ))}
              </div>
            )}
          </div>
          <div className="space-y-2 mt-4">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-200">Link do vídeo (opcional)</label>
            <input
              id="youtubeUrl"
              name="youtubeUrl"
              type="url"
              ...
              className="w-full border rounded-md px-3 py-2 text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-900 border-gray-300 dark:border-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
